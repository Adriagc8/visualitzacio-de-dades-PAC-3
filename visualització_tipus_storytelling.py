...
def create_graph5b_booking_changes(df):
    ...
    # Afegir anotació destacant la diferència (invertida: locals tenen MENYS canvis)
    if diff_pct < 0:  # Si locals tenen menys canvis (diff_pct negatiu)
        fig.add_annotation(
            text=f"<b>Les reserves locals presenten<br>{abs(diff_pct):.1f}% MENYS canvis<br>que les internacionals</b>",
            xref="paper", yref="paper",
            x=0.98, y=0.95,
            xanchor="right", yanchor="top",
    ...
    # Afegir anotació sobre reserves sense canvis (locals tenen MÉS sense canvis)
    if diff_pct_0 < 0:  # Si locals tenen més sense canvis (diff_pct_0 negatiu significa intl té més)
        fig.add_annotation(
            text=f"<b>Les reserves sense canvis (0)<br>són {abs(diff_pct_0):.1f}% MÉS freqüents<br>entre locals</b>",
            xref="paper", yref="paper",
            x=0.02, y=0.95,
            xanchor="left", yanchor="top",
    ...
    fig.update_layout(
        title={
            'text': 'Distribució de canvis a la reserva<br><sub>Les reserves locals presenten menys canvis que les internacionals, suggerint que la major cancel·lació no es deu a volatilitat en la reserva</sub>',
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 18}
        },
...
def create_graph5c_deposit_type(df):
    ...
    # Afegir anotació destacant la paradoxa (locals tenen MÉS Non Refund)
    if diff_nr < 0:  # Si locals tenen més Non Refund (diff_nr negatiu)
        fig.add_annotation(
            text=f"<b>Les reserves locals tenen<br>{abs(diff_nr):.1f}% MÉS dipòsits<br>Non Refund (més compromís)</b><br><br>Paradoxa: tot i tenir més compromís,<br>les locals cancel·len més",
            xref="paper", yref="paper",
            x=0.02, y=0.98,
            xanchor="left", yanchor="top",
    ...
    elif diff_nr > 0:  # Si internacionals tenen més Non Refund
        fig.add_annotation(
